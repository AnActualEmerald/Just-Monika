...
async function search(message, q_string, bot){
    ...
    if(tries >= 10) {message.channel.stopTyping(true);message.chanel.send("I'm a little busy right now, try again in a bit"); tries = 0; return false; }
    ...
    gr_client.searchBooks( { q:q_string, page: 1, field: 'title' } ).then(res => { //here we go
        ...
        gr_client.showBook(id).then(b => {
            ...
            bot.logger.error(`Caught error in when sending goodreads request: `);
